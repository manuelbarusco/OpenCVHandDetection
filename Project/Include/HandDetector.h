...
class HandDetector {
	...
	public:
		HandDetector(const cv::Mat& img, const std::string& path_file, cv::dnn::Net& net);
		void show_images(const std::vector<cv::Mat>& images);
		std::vector<cv::Mat> forward_process();

		std::vector<cv::Rect> post_process(std::vector<cv::Mat>& outputs, std::vector<std::string>& class_name);
